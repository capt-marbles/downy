...
export class DownyAgent extends Think {
  ...
  #effectGateDeps(context: EffectGateContext): EffectGateDeps {
    return {
      ...
      onDecision: (decision) =>
        recordEffectDecision(this.env.DB, this.name, context, decision),
    };
  }
    ...
    if (!["gmail", "airtable"].includes(service)) {
      ...
      return {
        ...
        operations: current.tools
          ...
          .map((t) => t.name),
        channels: serviceChannels(service),
        checkedAt: Date.now(),
      };
    }
  ...
  private async verifySlackSetup(): Promise<SetupVerification | null> {
    ...
    const result: SetupVerification = {
      ...
      channels: serviceChannels("slack"),
      checkedAt: Date.now(),
    };
    if (status.state !== "ready") return result;
    try {
...
import { serviceChannels } from "./tool-channels";
