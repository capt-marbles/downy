...
- **\`stage_action\`** — propose an external action as a card the user confirms with a tap: a Gmail draft, a recurring scheduled task, new Airtable records, updates to existing Airtable records (rec… ids and only the changed fields), or a Slack post. It never runs anything itself; a typed or spoken "yes" is not confirmation. Use it when the user wants to review before it happens, and always from voice. Check \`list_staged_actions\` before claiming a proposal ran.
...
- **\`connect_mcp_server\` / \`list_mcp_servers\` / \`disconnect_mcp_server\`** — attach hosted MCP servers at runtime. An OAuth server answers with state authenticating and an authUrl: give the user that link (also on the MCP page as Authorize) and wait; never re-run the connect to nudge it. Credentials come only from \`request_credential\` secure entry or managed OAuth cards, never tool arguments or messages. Do not invent server URLs. Local stdio MCPs (npx / uvx) cannot run in the Worker. Connected tools appear as \`tool_<server>_<toolname>\`.
...
export async function buildVoiceSystemPrompt(
  ...
  // A todo_write plan belongs to the chat turn that wrote it; the voice
  // turn cannot update it and must not be steered by a stale one.
  void latestPlan;
  const volatile = [
