...
import { runServiceSetup } from "../../runbooks/service-setup";
...
function fixture() {
  ...
  const stub = {
    showGmailConnectCard: vi.fn(async () => {}),
    showAirtableConnectCard: vi.fn(async () => {}),
    ...
    serviceSetupStatus: vi.fn(async () => []),
    runServiceSetup: (query: string, retry: boolean) =>
      runServiceSetup(query, retry, {
        load: async () => undefined,
        save: async () => {},
        verify: async () => null,
        // eslint-disable-next-line typescript/no-unsafe-type-assertion -- known service discovery needs no bindings.
        discover: (q) => findToolSetup({} as Cloudflare.Env, q),
        showCard: (name) =>
          name === "gmail"
            ? stub.showGmailConnectCard()
            : stub.showAirtableConnectCard(),
      }),
  };
...
it("Airtable setup displays a persistent OAuth card without asking for credentials or probing an endpoint", async () => {
  const f = fixture();
  const setup = createFindToolSetupTool(f.agent);
...
it("the conversation's Gmail setup tool displays a card with an explicit wait instruction, without connecting", async () => {
  const f = fixture();
  const tool = createFindToolSetupTool(f.agent);
  const result = await tool.execute!(
    { query: "Connect my Gmail for reading and drafts" },
    options,
  );
  expect(result).toMatchObject({
    runbook: { service: "gmail", step: "awaiting_authorization" },
  });
  expect(JSON.stringify(result)).toContain("Stop and wait");
