...
const PREAMBLE = [
  "## Skills",
  "",
  "Reusable instruction packs you've created or the user has built. Each lives at `skills/<name>/SKILL.md` in this workspace. When a skill's description matches the request, call `read_skill({ name })` once to load its parsed body (or use the workspace `read` tool on the SKILL.md path) and follow its instructions. A skill already read in this conversation stays in effect; do not read it again.",
  "",
].join("\n");
