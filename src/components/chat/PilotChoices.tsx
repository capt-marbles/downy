...
export function PilotChoicesButton({ slug }: { slug: string }) {
  const request = useMutation({
    mutationFn: async () => {
      const response = await agentFetch(slug, "/api/pilot-choices", {
        method: "POST",
      });
      if (!response.ok)
        throw new Error("Could not show pilot options. Try again.");
      return ReplySchema.parse(await response.json()).choice;
    },
    onSuccess: (choice) => {
      if (choice)
        document
          .getElementById(`pilot-choice-${choice.id}`)
  ...
  return (
    <div className="mb-2">
      <button
        ...
        className="btn btn-ghost btn-sm min-h-11 w-full justify-start gap-2 border border-base-300 text-sm font-normal"
        onClick={() => request.mutate()}
      >
        <ListChecks size={15} />
        {request.isPending ? "Preparing pilot options…" : "Choose a CUA pilot"}
      </button>
      {request.error && (
